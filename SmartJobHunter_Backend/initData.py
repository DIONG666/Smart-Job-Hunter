...
def insert_data_from_csv(csv_filepath):
    ...
    with open(csv_filepath, 'r', encoding='utf-8') as file:
        ...
        for row in csv_reader:
            ...
            data_to_insert = [row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7], row[8], row[9],
                              skills_json, city]
            cursor.execute('''
...
# 指定CSV文件路径
csv_file_path = 'globalData/predefinedInfo/data_all.csv'
# 调用函数，导入数据
insert_data_from_csv(csv_file_path)
# 导出数据到JSON文件
export_data_to_json('globalData/all_info.json')
