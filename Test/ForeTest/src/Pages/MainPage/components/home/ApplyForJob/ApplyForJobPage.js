...
const ApplyForJobPage=()=>{
    ...
    useEffect(() => {
        axios({
            method:'get',
            url:'http://127.0.0.1:5000/jobs/recommended/'+user.user_id,
        }).then(
            res=>{
                setJob(res.data)
                setAllLoading(false)
    ...
    useEffect(()=>{
        ...
        axios({
            method:'get',
            url:`http://127.0.0.1:5000/jobs/evaluation/${user.user_id}/${selectedJob.id}`
        }).then(
            res=>{
                        ...
                        <div style={{width:'100%',backgroundColor:'white',height:'65px',display:'flex',alignItems:'center'}}>
                            ...
                            <div style={{marginLeft:'3%',width:'30%'}}>
                                <RadioGroup onChange={value => {
                                    axios({
                                        method:'get',
                                        url:`http://127.0.0.1:5000/jobs/sort/${user.user_id}/${value}`,
                                    }).then(
                                        res=>{
                                            setJob(res.data)
                                            setSelectedJob({
